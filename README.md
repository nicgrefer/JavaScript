...
- **Tipos de Datos**: JavaScript es un lenguaje dinámico, lo que significa que no es necesario declarar el tipo de dato al crear una variable.
  ```javascript
...
- **Bucles**: Uso de bucles `for` y `while` para repetir acciones múltiples veces.
  ```javascript
  for (var i = 0; i < 10; i++) {
      document.write("<br>" + i);
  }
...
Estos apuntes teóricos cubren los conceptos y ejemplos clave encontrados en los archivos proporcionados. Si necesitas más detalles o tienes alguna otra solicitud, házmelo saber.
